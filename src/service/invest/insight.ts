...
export class InvestInsightService {
   ...
   */
  async bias() {
    // 文案不出现内部验证编号（owner 2026-08-08 呈现契约）；口径出处见 invest-model 提案文档
    const VERDICT =
      '乖离率是读数，不是信号。六个口径已全部验证失败：全历史分位高尾（进前 5% 后' +
      '未来 20 日反而 +0.53~+5.70pp）· 分位低尾（与价格闸几乎不相交）· 全历史' +
      '排名双尾（60 日完全反转）· 排名+止盈止损（胜率好但期望差、十年不出手）· ' +
      '近十年滚动排名状态机（七腿跑输买入持有 5.35~7.83pp）· 滚动 z 分数短线' +
